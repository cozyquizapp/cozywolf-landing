...
body{margin:0;background:#0A0814;color:#F1F5F9;font-family:Nunito,system-ui,sans-serif;-webkit-font-smoothing:antialiased;overflow-x:hidden}
...
class MobileOnePageInner extends Component<{ lang: Lang }, MOPState> {
  ...
  kicker(num: string, label: string) {
    return (
      <div data-rv="" style={sx('display:flex;align-items:center;gap:10px;margin:0 0 10px;font-size:10.5px;font-weight:900;letter-spacing:.2em;text-transform:uppercase;color:#FA4BA3')}>
        ...
        <span style={sx('color:#64748b')}>{label}</span>
      </div>
    );
  }
  ...
  bullet(text: string, color: string) {
    return (
      <span key={text} style={sx('display:flex;gap:11px;font-size:15px;line-height:1.5;font-weight:700;color:#e2e8f0')}>
  ...
  card(active: boolean, accent: string) {
    return `padding:20px;border-radius:22px;background:${active ? 'rgba(250,75,163,.06)' : 'rgba(255,255,255,.03)'};border:1px solid ${active ? accent : 'rgba(255,255,255,.09)'};cursor:pointer;transition:background .45s ${EASE},border-color .45s ${EASE}`;
  }
  ...
  plus(open: boolean, color: string) {
    return `flex:none;width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:19px;font-weight:900;color:${color};background:rgba(255,255,255,.06);transform:rotate(${open ? 45 : 0}deg);transition:transform .5s ${EASE}`;
  }
  ...
  renderHeader() {
    const L = this.T;
    const s = this.state;
    const langBtn = (on: boolean) => `min-height:38px;padding:0 13px;border:0;border-radius:999px;font-size:12.5px;font-weight:900;cursor:pointer;background:${on ? '#FA4BA3' : 'transparent'};color:${on ? '#0A0814' : '#94a3b8'}`;
    return (
      <header style={sx(`position:sticky;top:0;z-index:40;padding:12px 18px;overflow:visible;transition:background .5s ease,border-color .5s ease,backdrop-filter .4s ease;`
        + (s.scrolled
          ? 'background:rgba(10,8,20,.88);backdrop-filter:blur(14px);border-bottom:1px solid rgba(250,75,163,.14)'
        ...
        <div style={sx('position:relative;display:flex;align-items:center;gap:12px')}>
          <a href="#top" style={sx('display:flex;align-items:center;min-height:44px;gap:9px')}>
            ...
            <span style={sx("font-family:'League Spartan',sans-serif;font-weight:900;font-size:19px;color:#F1F5F9")}>CozyWolf</span>
          </a>
          <div style={sx('margin-left:auto;display:flex;align-items:center;gap:8px')}>
            <div style={sx('display:flex;padding:3px;border-radius:999px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.09)')}>
            ...
            <button type="button" aria-label={L.nav.menuLabel} aria-expanded={!!s.menu} onClick={() => this.setState(p => ({ menu: !p.menu }))}
              style={sx('flex:none;width:44px;height:44px;border-radius:14px;border:1px solid rgba(255,255,255,.09);background:rgba(255,255,255,.04);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;cursor:pointer')}>
              <span style={sx(`width:18px;height:2px;border-radius:2px;background:#cbd5e1;transition:transform .4s ${EASE};transform:translateY(${s.menu ? '3.5px' : '0'}) rotate(${s.menu ? 45 : 0}deg)`)}></span>
              <span style={sx(`width:18px;height:2px;border-radius:2px;background:#cbd5e1;transition:transform .4s ${EASE};transform:translateY(${s.menu ? '-3.5px' : '0'}) rotate(${s.menu ? -45 : 0}deg)`)}></span>
            </button>
          </div>
        </div>
            ...
            {L.nav.items.map((it, i) => (
              <a key={it.href} href={it.href} onClick={() => this.setState({ menu: false })}
                style={sx(i === L.nav.items.length - 1
                  ...
                  : 'display:flex;align-items:center;min-height:50px;padding:0 18px;font-size:16px;font-weight:800;color:#e2e8f0;border-bottom:1px solid rgba(250,75,163,.12)')}>{it.label}</a>
            ))}
          </div>
        </nav>
      </header>
    ...
    return (
      <section id="top" style={sx('position:relative;overflow:hidden;margin-top:-69px')}>
        ...
        <div style={sx('position:relative;margin-top:-46px;padding:0 20px 34px')}>
          ...
          <p style={sx(`margin:16px 0 0;font-size:16.5px;line-height:1.55;font-weight:600;color:#cbd5e1;text-wrap:pretty;animation:mRise .8s ${EASE} both .1s`)}>{L.hero.sub}</p>
          <p style={sx(`margin:10px 0 0;font-size:15px;line-height:1.55;font-weight:700;color:#94a3b8;text-wrap:pretty;animation:mRise .8s ${EASE} both .14s`)}>{L.hero.sub2}</p>
          <div style={sx(`margin-top:24px;display:flex;flex-direction:column;gap:11px;animation:mRise .8s ${EASE} both .18s`)}>
            <a href="#anfragen" onClick={() => this.setState({ tab: 'test', formStatus: 'idle' })}
              style={sx('display:flex;flex-direction:column;align-items:center;gap:3px;padding:16px 20px;border-radius:18px;background:#FA4BA3;color:#0A0814;font-weight:900;font-size:17px;min-height:56px;box-sizing:border-box;justify-content:center;box-shadow:0 10px 26px rgba(250,75,163,.28)')}>
            ...
            <div style={sx('display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap')}>
              <span style={sx('font-size:14.5px;font-weight:600;color:#94a3b8')}>{L.hero.bookQ}</span>
              <a href="#anfragen" onClick={() => this.setState({ tab: 'event', formStatus: 'idle' })}
                style={sx('display:inline-flex;align-items:center;min-height:44px;padding:0 16px;border-radius:999px;border:1px solid rgba(250,75,163,.34);color:#FFC7E4;font-weight:800;font-size:14.5px')}>{L.hero.bookCta}</a>
            </div>
          </div>
          <div style={sx('margin-top:22px;display:flex;align-items:center;gap:9px;font-size:14px;font-weight:700;letter-spacing:.01em;color:#a7b3c4')}>
  ...
  renderModes() {
    ...
    return (
      <section id="spielarten" style={sx('padding:14px 20px 42px;border-top:1px solid rgba(250,75,163,.07)')}>
        ...
        <p data-rv="" style={sx('margin:0 0 22px;font-size:15.5px;line-height:1.6;color:#94a3b8;font-weight:600;text-wrap:pretty')}>{L.modes.sub}</p>
        ...
        <div data-rv="" style={sx('display:flex;flex-direction:column;gap:14px')}>
          <div onClick={() => this.open(s.open === 'quiz' ? null : 'quiz')} style={sx(this.card(s.open === 'quiz', 'rgba(250,75,163,.45)'))}>
            <div style={sx('display:flex;align-items:center;gap:12px')}>
              ...
              <span style={sx('margin-left:auto;padding:5px 11px;border-radius:999px;background:rgba(250,75,163,.14);border:1px solid rgba(250,75,163,.3);font-size:11.5px;font-weight:900;color:#FFC7E4;white-space:nowrap')}>{L.modes.quizChip}</span>
              <span style={sx(this.plus(s.open === 'quiz', '#FA4BA3'))}>+</span>
            </div>
            <p style={sx('margin:10px 0 0;font-size:15px;line-height:1.55;color:#cbd5e1;font-weight:600;text-wrap:pretty')}>{L.modes.quizP}</p>
            <div style={sx('margin-top:14px;display:flex;align-items:center;gap:9px')}>
              <div style={sx('display:flex')}>
                {TEAMS.map((t, i) => (
                  <span key={t.id} style={sx(`width:32px;height:32px;border-radius:50%;flex:none;background:${t.color} url(${t.av}) center/76% no-repeat;border:2px solid #0f0a1a;margin-left:${i ? '-10px' : '0'};box-shadow:0 3px 8px rgba(0,0,0,.45)`)}></span>
                ))}
              </div>
              <span style={sx('font-size:12.5px;font-weight:800;letter-spacing:.04em;color:#64748b')}>{L.modes.quizTeams}</span>
            </div>
            <div style={sx(this.bodyStyle(s.open === 'quiz'))}>
              <div style={sx('min-height:0;overflow:hidden;padding-top:16px;display:flex;flex-direction:column;gap:12px')}>
                ...
                <div style={sx('width:100%;max-width:212px;margin:4px auto 0')}>
                  <div style={sx(`padding:9px;border-radius:16px;background:rgba(255,255,255,.015);border:1.5px solid ${actTeam ? actTeam.color : 'rgba(255,255,255,.1)'};box-shadow:${actTeam ? '0 0 24px ' + actTeam.color + '44' : 'none'};transition:border-color .5s ease,box-shadow .5s ease`)}>
                    <div style={sx('display:grid;grid-template-columns:repeat(5,1fr);gap:4px')}>
                      {owner.map((id, i) => {
                        ...
                        if (!t) return <span key={i} style={sx(cellBase + `border-radius:${RAD}px;background:rgba(255,255,255,.028);border:1px solid rgba(255,255,255,.05)`)}></span>;
                        ...
                        const shadow = [
                          nT ? '' : 'inset 0 1px 0 rgba(255,255,255,.22)',
                ...
                <div style={sx(`margin-top:12px;min-height:38px;display:flex;align-items:center;justify-content:center;text-align:center;font-size:13.5px;font-weight:900;line-height:1.4;color:${s.act < 0 ? '#64748b' : (actTeam ? actTeam.color : '#64748b')};transition:color .4s ease`)}>
          ...
          <div onClick={() => this.open(s.open === 'arena' ? null : 'arena')} style={sx(this.card(s.open === 'arena', 'rgba(96,165,250,.45)'))}>
            ...
            <p style={sx('margin:10px 0 0;font-size:15px;line-height:1.55;color:#cbd5e1;font-weight:600;text-wrap:pretty')}>{L.modes.arenaP}</p>
            <div style={sx('margin-top:14px;display:flex;align-items:center;gap:9px')}>
              <div style={sx('display:flex')}>
                {L.factions.map((fa, i) => (
              ...
              <span style={sx('font-size:12.5px;font-weight:800;letter-spacing:.04em;color:#64748b')}>{L.modes.arenaFactions}</span>
            </div>
            <div style={sx(this.bodyStyle(s.open === 'arena'))}>
              <div style={sx('min-height:0;overflow:hidden;padding-top:16px;display:flex;flex-direction:column;gap:12px')}>
                {L.modes.arenaBullets.map(b => this.bullet(b, '#60A5FA'))}
                <div style={sx('margin-top:4px;padding:12px;border-radius:16px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);display:flex;flex-direction:column;gap:8px')}>
                  {L.factions.slice(0, 3).map((b, i) => (
                    <div key={b.file} style={sx('display:flex;align-items:center;gap:9px')}>
                      ...
                      <span style={sx('flex:1;min-width:0;display:flex;flex-direction:column;gap:3px')}>
                        <span style={sx('font-size:12.5px;font-weight:900;color:#e2e8f0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis')}>{b.name}</span>
                        <span style={sx('height:6px;border-radius:999px;background:rgba(255,255,255,.07);overflow:hidden')}>
  ...
  renderProbe() {
        ...
        <>
          ...
          <div style={sx('margin-top:16px;display:flex;flex-direction:column;gap:9px')}>
            {C.opts.map((o, i) => {
              ...
              const bg = !done ? 'rgba(255,255,255,.04)' : right ? 'rgba(52,211,153,.14)' : chosen ? 'rgba(248,113,113,.12)' : 'rgba(255,255,255,.03)';
              const bd = !done ? 'rgba(255,255,255,.1)' : right ? 'rgba(52,211,153,.5)' : chosen ? 'rgba(248,113,113,.45)' : 'rgba(255,255,255,.07)';
              return (
                <button key={i} type="button" onClick={() => { if (s.picked !== null) return; this.setState({ picked: i }); this.nextCat(4200); }}
                  style={sx(`display:flex;align-items:center;gap:12px;width:100%;box-sizing:border-box;min-height:54px;padding:0 14px;border-radius:15px;cursor:pointer;text-align:left;background:${bg};border:1.5px solid ${bd};color:#F1F5F9;transition:background .35s ease,border-color .35s ease`)}>
    ...
    if (C.kind === 'guess') {
      ...
      hintCol = done && near ? '#34D399' : '#94a3b8';
      body = (
        <>
          ...
          <div style={sx('margin-top:16px;display:flex;align-items:baseline;justify-content:center;gap:8px')}>
            ...
            <span style={sx('font-size:14px;font-weight:800;color:#94a3b8')}>{L.probe.bones}</span>
          </div>
          ...
          <button type="button" onClick={() => { if (done) return; this.setState({ guessDone: true }); this.nextCat(4200); }}
            style={sx(`width:100%;min-height:50px;margin-top:10px;border:0;border-radius:15px;cursor:pointer;font-size:15.5px;font-weight:900;background:${done ? 'rgba(255,255,255,.06)' : '#F59E0B'};color:${done ? '#cbd5e1' : '#0A0814'}`)}>
      ...
      body = (
        <>
          ...
          <div style={sx('margin-top:16px;display:flex;flex-direction:column;gap:10px')}>
            {C.opts.map((o, i) => (
              <div key={i} style={sx(`display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:15px;background:${s.ptsDone && i === C.correct ? 'rgba(52,211,153,.14)' : 'rgba(255,255,255,.04)'};border:1.5px solid ${s.ptsDone && i === C.correct ? 'rgba(52,211,153,.5)' : 'rgba(255,255,255,.1)'};transition:background .4s ease,border-color .4s ease`)}>
          ...
          <div style={sx(`margin-top:12px;text-align:center;font-size:13px;font-weight:900;letter-spacing:.06em;color:${left > 0 ? '#94a3b8' : '#22C55E'}`)}>
    ...
    return (
      <section id="probieren" style={sx('padding:36px 20px 42px;border-top:1px solid rgba(250,75,163,.07);background:radial-gradient(ellipse at 50% 0%,rgba(250,75,163,.1),transparent 62%)')}>
        ...
        {s.splash && (
          <div role="status" aria-live="polite" onClick={skip}
            ...
            <div style={sx('margin-top:8px;font-size:15px;line-height:1.5;font-weight:800;color:#cbd5e1;text-align:center;text-wrap:pretty')}>{N.claim}</div>
            ...
            <div style={sx('margin-top:12px;font-size:12.5px;font-weight:800;letter-spacing:.06em;color:#64748b')}>{L.probe.skipHint}</div>
          </div>
        )}
  ...
  renderAnlaesse() {
    const L = this.T;
    return (
      <section id="anlaesse" style={sx('padding:36px 20px 42px;border-top:1px solid rgba(250,75,163,.07)')}>
        ...
        <p data-rv="" style={sx('margin:0 0 22px;font-size:15.5px;line-height:1.6;color:#94a3b8;font-weight:600;text-wrap:pretty')}>{L.anlaesse.sub}</p>
        <div data-rv="" style={sx('display:flex;flex-direction:column;gap:12px')}>
          {L.anlaesse.cards.map((c, i) => (
            <div key={i} style={sx('padding:20px;border-radius:20px;background:rgba(255,255,255,.03);border:1px solid rgba(250,75,163,.2)')}>
              ...
              <p style={sx('margin:0 0 14px;font-size:14.5px;line-height:1.6;color:#cbd5e1;font-weight:600;text-wrap:pretty')}>{c.p}</p>
              <a href="#anfragen" onClick={() => this.setState({ tab: 'event', formStatus: 'idle', anlass: this.T.form.anlassOpts[Math.min(i, 2)] })}
                style={sx('display:inline-flex;align-items:center;min-height:44px;padding:0 18px;border-radius:999px;background:rgba(250,75,163,.14);border:1px solid rgba(250,75,163,.4);color:#FFC7E4;font-weight:900;font-size:14.5px')}>{L.anlaesse.cta}</a>
            </div>
          ))}
        </div>
      </section>
  ...
  renderAblauf() {
    ...
    const s = this.state;
    return (
      <section id="ablauf" style={sx('padding:36px 20px 42px;border-top:1px solid rgba(250,75,163,.07)')}>
        ...
        <p data-rv="" style={sx('margin:0 0 14px;font-size:15.5px;line-height:1.6;color:#94a3b8;font-weight:600')}>{L.ablauf.sub}</p>
        <div data-rv="" style={sx('display:inline-flex;align-items:baseline;gap:9px;padding:11px 18px;border-radius:999px;background:rgba(250,75,163,.12);border:1px solid rgba(250,75,163,.42);margin:0 0 20px')}>
        ...
        <div ref={el => { this._wall = el; }} data-rv="" style={sx('position:relative;overflow:hidden;margin:0 -20px 26px')}>
          ...
          <div style={sx(`position:absolute;left:29.1%;top:13.7%;width:44.7%;height:31.4%;border-radius:8px;overflow:hidden;container-type:inline-size;display:flex;align-items:center;justify-content:center;background:${s.wallOn ? '#0b0714' : 'transparent'};box-shadow:${s.wallOn ? '0 0 46px rgba(255,242,250,.2)' : 'none'};transition:background 1.1s ease,box-shadow 1.3s ease`)}>
            <div style={sx(`display:flex;flex-direction:column;align-items:center;opacity:${s.wallOn ? 1 : 0};transform:scale(${s.wallOn ? 1 : .92});transition:opacity .9s ease ${s.wallOn ? '1.15s' : '0s'},transform 1.1s ${EASE} ${s.wallOn ? '1.15s' : '0s'}`)}>
              <div style={sx('display:flex;flex-direction:column;align-items:center;gap:.4cqw;padding:2cqw 5cqw;border-radius:3cqw;border:1px solid rgba(250,75,163,.35);background:rgba(250,75,163,.06)')}>
              ...
              <div style={sx('display:flex;align-items:center;gap:2.4cqw;margin-top:2.2cqw')}>
                ...
                <span style={sx('padding:1.6cqw 2.8cqw;border-radius:2.4cqw;border:1px solid rgba(250,75,163,.45);background:rgba(255,255,255,.03);font-size:3cqw;font-weight:900;line-height:1.25;color:#F1F5F9;text-align:center;white-space:nowrap')}>{L.ablauf.welcomeSub}</span>
              </div>
            </div>
          </div>
        </div>
        <div data-rv="" style={sx('padding:20px;border-radius:20px;background:rgba(250,75,163,.07);border:1px solid rgba(250,75,163,.28);display:flex;flex-direction:column;gap:16px')}>
          <div style={sx('display:flex;gap:12px')}>
            ...
            <span style={sx('flex:1;font-size:15.5px;line-height:1.55;font-weight:700;color:#e2e8f0')}><strong style={sx('color:#FFC7E4')}>{L.ablauf.bringT}</strong> {L.ablauf.bring}</span>
          </div>
          <div style={sx('display:flex;gap:12px')}>
            <span style={sx('flex:none;width:7px;height:7px;border-radius:50%;background:#64748b;margin-top:9px')}></span>
            <span style={sx('flex:1;font-size:15.5px;line-height:1.55;font-weight:700;color:#cbd5e1')}><strong style={sx('color:#F1F5F9')}>{L.ablauf.needT}</strong> {L.ablauf.need}</span>
          </div>
        </div>
      </section>
    );
  ...
  renderJohannes() {
    ...
    return (
      <section id="johannes" style={sx('padding:36px 20px 42px;border-top:1px solid rgba(250,75,163,.07)')}>
  ...
  renderFaq() {
    const L = this.T;
    return (
      <section id="fragen" style={sx('padding:36px 20px 42px;border-top:1px solid rgba(250,75,163,.07)')}>
        ...
        <div data-rv="" style={sx('display:flex;flex-direction:column;gap:10px')}>
          {L.faq.items.map((item, i) => (
            <details key={i} style={sx('border-radius:16px;background:rgba(255,255,255,.03);border:1px solid rgba(250,75,163,.18);overflow:hidden')}>
              ...
              <div style={sx('padding:0 18px 18px;font-size:15px;line-height:1.65;font-weight:600;color:#cbd5e1')}>{item.a}</div>
            </details>
          ))}
        </div>
      </section>
  ...
  renderForm() {
    ...
    const tab = (on: boolean) => `min-height:46px;padding:0 12px;border:0;border-radius:999px;font-size:14px;font-weight:900;cursor:pointer;background:${on ? '#FA4BA3' : 'transparent'};color:${on ? '#0A0814' : '#94a3b8'};transition:background .3s ease,color .3s ease`;
    const inputStyle = 'width:100%;box-sizing:border-box;min-height:52px;padding:0 14px;border-radius:14px;background:rgba(255,255,255,.05);border:1.5px solid rgba(250,75,163,.24);color:#F1F5F9;font-size:16px;font-weight:700;outline:none';
    const labelStyle = 'display:flex;flex-direction:column;gap:7px;font-size:12.5px;font-weight:900;letter-spacing:.1em;text-transform:uppercase;color:#94a3b8';
    return (
      <section id="anfragen" style={sx('padding:36px 20px 46px;border-top:1px solid rgba(250,75,163,.07);background:radial-gradient(ellipse at 50% 0%,rgba(250,75,163,.14),transparent 66%)')}>
        ...
        <p data-rv="" style={sx('margin:0 0 8px;font-size:16px;line-height:1.6;font-weight:600;color:#cbd5e1;text-wrap:pretty')}>{L.form.sub}</p>

        <div style={sx('display:grid;grid-template-columns:1fr 1fr;gap:6px;padding:5px;border-radius:999px;background:rgba(255,255,255,.04);border:1px solid rgba(250,75,163,.22);margin-bottom:18px')}>
        ...
        <div style={sx('display:flex;align-items:center;gap:12px;padding:14px 16px;border-radius:18px;background:rgba(250,75,163,.1);border:1px solid rgba(250,75,163,.35);margin-bottom:18px')}>
          ...
          <span style={sx('flex:1;min-width:0;font-size:13.5px;font-weight:700;line-height:1.45;color:#cbd5e1')}>
        ...
        {st === 'ok' && (
          <div role="status" style={sx('padding:24px 20px;border-radius:22px;background:rgba(255,255,255,.03);border:1.5px solid rgba(250,75,163,.24);text-align:center')}>
            <div style={sx('font-size:19px;font-weight:900')}>{L.form.okTitle}</div>
            <p style={sx('margin:8px 0 0;font-size:15px;line-height:1.6;color:#cbd5e1;font-weight:600')}>{L.form.okBody}</p>
          </div>
        )}

        {st !== 'ok' && (
          <form onSubmit={this.submitForm} style={sx('padding:18px;border-radius:22px;background:rgba(255,255,255,.03);border:1.5px solid rgba(250,75,163,.24);display:flex;flex-direction:column;gap:12px')}>
            ...
            <div style={sx('display:grid;grid-template-columns:1fr 108px;gap:12px')}>
              <label style={sx(labelStyle + ';min-width:0')}>{L.form.anlass}
                <select name="anlass" value={s.anlass || L.form.anlassOpts[0]} onChange={e => this.setState({ anlass: e.target.value })} style={sx(inputStyle)}>
                  {L.form.anlassOpts.map(o => <option key={o} value={o} style={sx('background:#171126;color:#F1F5F9')}>{o}</option>)}
                </select>
              </label>
            ...
            <button type="submit" style={sx('min-height:56px;border:0;border-radius:18px;background:#FA4BA3;color:#0A0814;font-size:17px;font-weight:900;box-shadow:0 10px 26px rgba(250,75,163,.28);cursor:pointer')}>
              {st === 'sending' ? L.form.sending : L.form.submit}
            </button>
            <p style={sx('margin:0;font-size:12.5px;line-height:1.5;color:#64748b;font-weight:600')}>
    ...
    return (
      <div style={sx('max-width:520px;margin:0 auto;position:relative;overflow:hidden;background:#0A0814;padding-bottom:96px')}>
        ...
        <div inert={s.menu || undefined}>
          ...
          {this.renderFaq()}
          {this.renderForm()}
          <footer style={sx('padding:28px 20px 34px;border-top:1px solid rgba(250,75,163,.1);display:flex;flex-direction:column;gap:16px')}>
            ...
            <div style={sx('display:flex;flex-wrap:wrap;gap:8px')}>
              <a href="/impressum" style={sx('display:inline-flex;align-items:center;min-height:44px;padding:0 14px;border-radius:12px;background:rgba(255,255,255,.04);border:1px solid rgba(250,75,163,.18);font-size:14.5px;font-weight:800')}>{L.footer.imprint}</a>
              <a href="/datenschutz" style={sx('display:inline-flex;align-items:center;min-height:44px;padding:0 14px;border-radius:12px;background:rgba(255,255,255,.04);border:1px solid rgba(250,75,163,.18);font-size:14.5px;font-weight:800')}>{L.footer.privacy}</a>
              <a href={INSTA_URL} style={sx('display:inline-flex;align-items:center;min-height:44px;padding:0 14px;border-radius:12px;background:rgba(255,255,255,.04);border:1px solid rgba(250,75,163,.18);font-size:14.5px;font-weight:800')}>{L.footer.instagram}</a>
            </div>
            <div style={sx('font-size:12.5px;line-height:1.6;color:#64748b;font-weight:600')}>{L.footer.aiNote}</div>
          </footer>
          ...
          <div style={sx('position:fixed;left:0;right:0;bottom:0;z-index:50;display:flex;justify-content:center;padding:10px 16px calc(10px + env(safe-area-inset-bottom));background:linear-gradient(180deg,rgba(10,8,20,0),rgba(10,8,20,.94) 42%);pointer-events:none')}>
            <a href="#anfragen" onClick={() => this.setState({ tab: 'test', formStatus: 'idle' })}
              style={sx('pointer-events:auto;width:100%;max-width:488px;display:flex;align-items:center;justify-content:center;gap:9px;min-height:54px;border-radius:999px;background:#FA4BA3;color:#0A0814;font-size:16.5px;font-weight:900;box-shadow:0 12px 30px rgba(250,75,163,.34)')}>
